...
const Toolbox = () => {
    ...
    const updateColor = (colorValue) => {
        dispatch(changeColor({ item: activeMenuItem, color: colorValue }));
        socket.emit('changeConfig', { color: colorValue, size });
    }
    ...
    return (
        <div className={styles.toolboxContainer}>
            ...
            {showBrushToolOption && <div className={styles.toolItem}>
                ...
                <div className={styles.itemContainer}>
                    <input type="range" min={0} max={10} step={1} onChange={updateBrushSize} />
                </div>
            </div>}
        </div>
    )
